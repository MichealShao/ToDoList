...
router.post('/', auth, async (req, res) => {
  ...
  try {
    ...
    try {
      // Directly process date string to avoid timezone conversion issues
    ...
    if (start_time && initialStatus !== 'Pending') {
      ...
      // Directly construct date string
  ...
  try {
    ...
    if (deadline) {
      try {
        ...
        console.log('Processed deadline date during update:', deadlineDateStr, ', converted to Date:', taskFields.deadline);
      } catch (error) {
      ...
      try {
        if (status === 'Pending' || task.status === 'Pending' && status === undefined) {
          ...
          try {
            ...
            console.log('Processed start_time date during update:', startTimeStr, ', converted to Date:', taskFields.start_time);
          } catch (error) {
            console.error('Error processing start_time date during update:', error);
