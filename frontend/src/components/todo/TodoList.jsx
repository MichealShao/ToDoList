...
function TodoList() {
  ...
  const openAddModal = () => {
    debounceOperation('addTask', () => {
      ...
      setFormData({
        priority: "Medium",
        deadline: todayDateStr, // Set default value to today
        hours: "",
        details: "",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const taskData = {
        ...
        // Ensure date only includes date part, not time part
      ...
      if (showEditModal) {
        ...
        const taskWithCreatedAt = {
          ...
          createdAt: getTodayDateString() // Use local date
        };
    ...
    return [...tasks]
      ...
      .map(task => {
        ...
        if (task.status !== 'Completed') {
          ...
          // Direct comparison of date strings, avoiding timezone issues
          // Only mark as expired when deadline is strictly less than today's date
